...
#	previ = datetime.datetime.now();
	kappa1 = c + ((k_limit*p)/2);
	sum1 = 0;
	for i in range(k_limit):
		sum1 = sum1 + av[i]*p + np.dot(am[i,:],am[i,:].T)
	kappa2 = d + (sum1/2);
#	curi = datetime.datetime.now();
#	print "Updating Sigma A Time: "+delta_time_string(previ,curi);
...
def update_all_zqk(vn, knownx, knowny, zm, am, av, n, p, q, k_limit, xxt_q, dgtaus, exp_phiphi, threshold):
    ...
    all_q = np.arange(0,q);
    new_nus = zm;
    # sum1s = np.dot(np.mat(exp_phiphi).T,np.mat(xxt_q));
    ...
    for k1 in range(k_limit):
        ...
        if (all(abs(phi_mean[k1,:]) < threshold)):
            ...
            for q1 in range(q):
                ...
                # sum2 = exp_phiphi[k1]*np.sum(np.dot(knownx[:,q1],np.dot(knownx[:,not_q],new_nus[not_q,k1])))
                sum3 = np.sum(np.dot(knownx[:,q1],resids));
                zeta = dgtaus[k1] - (1/(2*vn))*sum1 - (1/vn)*sum2 + (1/vn)*sum3;
                # zeta = dgtaus[k1] - (1/(2*vn))*sum1s[k1,q1] - (1/vn)*sum2 + (1/vn)*sum3;
                if (zeta < -50):
                    new_nus[q1,k1] = 0;
                elif (zeta > 50):
                    new_nus[q1,k1] = 1;
        ...
        if (np.all(new_nus == 0)):
            new_nus = update_all_zqk(vn, knownx, knowny, zm, am, av, n, p, q, k_limit, xxt_q, dgtaus, exp_phiphi, 0);
    return new_nus;

# Function to update the parameters on Akp when Akp has an L1 penalty
def update_all_ak_va(kappa1, kappa2, vn, knownx, knowny, am, zm, k_limit, n, p, q, xxt_qs, threshold):
    
    phi_means = am;
    phi_vars = np.zeros((k_limit));
    ...
    for k1 in range(k_limit):
        ...
        if (all(nu[:,k1] < threshold)):
            ...
            phi_vars[k1] = math.pow(((kappa1/kappa2) + (sumvs_k[k1]/vn)),-1);
            phi_means[k1,:] = (sump/vn)*phi_vars[k1];
    
    if (np.all(phi_means == 0)):
        phi_means,phi_vars = update_all_ak_va(kappa1, kappa2, vn, knownx, knowny, am, zm, k_limit, n, p, q, xxt_qs, 0);
...
def geweke(trace,alpha):
	...
	if (ntrace < 10):
		return False;
	#Check the convergence of a single parameter
	elif (len(np.shape(trace))==1):
		seta = np.zeros(ntrace,dtype=bool);
		setb = np.zeros(ntrace,dtype=bool);
		seta[int(ntrace/10):int(ntrace/5)] = True;
		setb[int(ntrace/2):ntrace-1] = True;
		...
		return pval > alpha;
	#Check the convergence of a one-dimensional set of parameters
	elif (len(np.shape(trace))==2):
		td = np.shape(trace)[1];
		od = np.shape(trace)[0]
		bvals = np.zeros(od,dtype=bool);
		for o in range(od):
			seta = np.zeros(td,dtype=bool);
			setb = np.zeros(td,dtype=bool);
			seta[int(td/10):int(td/5)] = True;
			setb[int(td/2):td-1] = True;
			try:
				z = (np.mean(trace[o,seta])-np.mean(trace[o,setb]))/np.sqrt((np.std(trace[o,seta])/sum(seta)) + (np.std(trace[o,setb])/sum(setb)));
			except (ZeroDivisionError,RuntimeWarning):
				z = np.nan;
			if (math.isnan(z) and (np.mean(trace[o,seta])-np.mean(trace[o,setb]))==0):
				z = 0;
			pval = scipy.stats.norm.sf(abs(z))*2;
			bvals[o] = pval > alpha;
		return all(bvals)
	#Check the convergence of a two-dimensional set of parameters
	else:
		td = np.shape(trace)[2];
		od = np.shape(trace)[0]*np.shape(trace)[1]
		newtraces = np.reshape(trace,(od,td));
		bvals = np.zeros(od,dtype=bool);
		for o in range(od):
			seta = np.zeros(td,dtype=bool);
			setb = np.zeros(td,dtype=bool);
			...
			setb[int(td/2):td-1] = True;
			try:
				z = (np.mean(newtraces[o,seta])-np.mean(newtraces[o,setb]))/np.sqrt((np.std(newtraces[o,seta])/sum(seta)) + (np.std(newtraces[o,setb])/sum(setb)));
			except (ZeroDivisionError,RuntimeWarning):
				z = np.nan;
			if (math.isnan(z) and (np.mean(newtraces[o,seta])-np.mean(newtraces[o,setb]))==0):
				z = 0;
			if ((np.var(newtraces[o,seta]) + np.var(newtraces[o,setb])) == 0):
				z = 0;
			pval = scipy.stats.norm.sf(abs(z))*2;
			bvals[o] = pval > alpha;
		return all(bvals)
...
def fitModel(iterations,x,y,p,q,k_limit,c,d,phi_mean,nu,alpha,var_a,var_n,learn_a,learn_z,learn_avar, threshold):
    
	#Initialize Parameters (A and Z might already be initialized so they are passed in as args)
	phi_var = np.multiply(np.ones((int(k_limit))),var_a);
	...
	phi_var_traces = np.zeros((int(k_limit),iterations));
	...
	for i in range(iterations):
		...
		if (learn_a):
			phi_mean,phi_var = update_all_ak_va(kappa1, kappa2, var_n, x, y, phi_mean, nu, k_limit, n, p, q, xxt_qs, threshold)
			for k1 in range(k_limit):
				exp_phiphis[k1] = (phi_var[k1]*p)+np.dot(phi_mean[k1,:],phi_mean[k1,:].T);
		phi_mean_traces[:,:,i] = phi_mean;
		phi_var_traces[:,i] = phi_var;
        # timez = datetime.datetime.now();
        # print "Updating A Time: "+delta_time_string(start,timez);
		# Update Pi
        # start = datetime.datetime.now();
		trace_varnus = 0;
		for k1 in range(k_limit):
        ...
        # timez = datetime.datetime.now();
        # print "Updating Pi Time: "+delta_time_string(start,timez);
		# Update Z
        # start = datetime.datetime.now();
		if (learn_z):
		...
		nu_traces[:,:,i] = nu;
        # timez = datetime.datetime.now();
        # print "Updating Z Time: "+delta_time_string(start,timez);
		#Update Sigma_A
        # start = datetime.datetime.now();
		if (learn_avar):
		...
		kappas[1,i] = kappa2;
        # timez = datetime.datetime.now();
        # print "Updating Sigma A Time: "+delta_time_string(start,timez);
		...
		if (i % 100 == 0):
		    ...
		    previ = curi;
		    con_tau1 = geweke(tau_traces1[:,0:i],0.05);
		    con_tau2 = geweke(tau_traces2[:,0:i],0.05);
		    nu_con = geweke(nu_traces[:,:,0:i],0.05);
		    phim_con = geweke(phi_mean_traces[:,:,0:i],0.05);
		    phiv_con = geweke(phi_var_traces[:,0:i],0.05);
		    avar_con = geweke(kappas[:,0:i],0.05);
		    if (con_tau1):
...
try:
	options, args = getopt.getopt(sys.argv[1:], "x:y:c:", ["alpha=","iter=","klimit=","ainit=","zinit=","vara=","varn=", "sac=", "sad=", "sa", "sz", "sva", "outdir=","maf=", "info=", "subset=", "thres=", "nr", "plot", "xval"]);
...
if (a_init != ""):
	...
	phi_mean = np.random.normal(0,1,(int(k_limit),int(p)));
...
if (setparams_xval==1):
	...
	if (learn_vara==1):
		...
		for xp in range(np.shape(xbrrr_l1_params)[0]):
			...
			for xvp in range(xval_parts):
				nu_traces,tau_traces1,tau_traces2,phi_mean_traces,phi_var_traces,kappas,numiter = fitModel(iterations,splitx[xvp],splity[xvp],p,q,k_limit,c,d,phi_mean,nu,xbrrr_l1_params[xp,0],var_a,xbrrr_l1_params[xp,1],learn_a,learn_z,learn_vara, threshold);
	...
	# Run the Model without the L1 Penalty on A
		...
		for xp in range(np.shape(xbrrr_params)[0]):
			...
			# Fit the model for each each parameter setting for xval partition
			for xvp in range(xval_parts):
				nu_traces,tau_traces1,tau_traces2,phi_mean_traces,phi_var_traces,kappas,numiter = fitModel(iterations,splitx[xvp],splity[xvp],p,q,k_limit,c,d,phi_mean,nu,xbrrr_params[xp,0],xbrrr_params[xp,1],xbrrr_params[xp,2],learn_a,learn_z,learn_vara,threshold);
				ey = (np.dot(splitx[xvp],np.dot(nu_traces[:,:,numiter-1],phi_mean_traces[:,:,numiter-1])));
				full_est_y[start:start+np.shape(ey)[0],:] = ey;
				start = start + np.shape(ey)[0];
			# Compute the RSS for this paramter setting
...
nu_traces,tau_traces1,tau_traces2,phi_mean_traces,phi_var_traces,kappas,numiter = fitModel(iterations,x,y,p,q,k_limit,c,d,phi_mean,nu,alpha,var_a,var_n,learn_a,learn_z,learn_vara,threshold);
...
if (learn_vara==1):
	var_a = kappas[1,numiter-1]/(kappas[0,numiter-1]-1);

# Write Out the Results
f = open(output_dir+"alpha.csv",'w')
f.write(str(alpha)) 
f.close()
f = open(output_dir+"var_a.csv",'w')
f.write(str(var_a)) 
f.close()
...
np.savetxt('phi_mean.out', phi_mean_traces[:,:,numiter-1], delimiter=',')
...
rank = np.sum(np.sum(sampled_z > threshold,axis=0)>0);
...
if (makeplots==1):
	...
	plt.clf()
	plt.plot(range(numiter),kappas[0,:]);
	plt.plot(range(numiter),kappas[1,:]);
	plt.ylabel('Kappa')
	plt.xlabel('Iteration')
	plt.title('Traces of Kappas')
	plt.savefig(output_dir+'TracesSigmaA.png')
